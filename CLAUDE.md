...
**The forms post to [`nieusync/website-api`](https://github.com/nieusync/website-api)**, on `website-api.nieusync.com`, through `src/api.ts`. Not `api.nieusync.com`: that is Praxis, the product the firm runs its business on, and a public form has no business writing to the database the clients and invoices live in. The site's own back end does two things and holds nothing else: `POST /api/contact` emails an enquiry, `POST /api/submissions` stores a tool's answers and emails a notice. Both answer a refusal with `{ error }` already written in Portuguese, so that string is shown verbatim rather than translated here. Adding a question to a tool is a change in this repo alone: the API stores the answers as an opaque blob and never reads them.
